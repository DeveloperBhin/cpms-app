...
class _FarmsPageState extends State<FarmsPage> {
  ...
  Future<void> _loadFarms() async {
    ...
    try {
      final localFarms = await LocalDataService.instance.getFarms();

      if (mounted && localFarms.isNotEmpty) {
        setState(() {
          _farms = List<Map<String, dynamic>>.from(localFarms);
        });
      }
    } catch (e) {
      debugPrint('FARMS PAGE LOCAL LOAD ERROR: $e');
    }
    ...
    try {
      ...
      final apiFarms = List<Map<String, dynamic>>.from(results[0]);

      final apiTrees = List<Map<String, dynamic>>.from(results[1]);

      debugPrint('FARMS PAGE -> FARMS: ${apiFarms.length}');

      debugPrint('FARMS PAGE -> TREES: ${apiTrees.length}');
      ...
      debugPrint('FARMS PAGE API LOAD ERROR: $e');
  ...
  List<Map<String, dynamic>> _treesForFarm(Map<String, dynamic> farm) {
  ...
  int _healthyTrees(List<Map<String, dynamic>> trees) {
    return trees.where((tree) {
      return tree['status']?.toString().toUpperCase() == 'HEALTHY';
    }).length;
  }

  int _diseasedTrees(List<Map<String, dynamic>> trees) {
    return trees.where((tree) {
      return tree['status']?.toString().toUpperCase() == 'DISEASED';
    }).length;
  }

  int _deadTrees(List<Map<String, dynamic>> trees) {
    return trees.where((tree) {
  ...
  Widget _featuredFarm(BuildContext context, Map<String, dynamic> farm) {
    ...
    final farmTrees = _treesForFarm(farm);

    final healthy = _healthyTrees(farmTrees);

    final diseased = _diseasedTrees(farmTrees);

    final dead = _deadTrees(farmTrees);
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _farmBadge(icon: Icons.category_outlined, label: farmType),

              _farmBadge(
                icon: Icons.landscape_outlined,
                label: '$acreage ${l10n.acres}',
              ),

              _farmBadge(
                icon: Icons.park_outlined,
                label: '${farmTrees.length} ${l10n.trees}',
              ),
            ],
          ),
          ...
          // PLANTING DATE
          // =======================================================
          if (_plantingDateText(context, farm).isNotEmpty)
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      _plantingDateText(context, farm).replaceFirst(' • ', ''),
                      style: const TextStyle(
                        color: textGrey,
                        fontSize: AppTextStyles.bodySmall,
                        fontWeight: FontWeight.w500,
  ...
  Widget _farmBadge({required IconData icon, required String label}) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(icon, color: primaryGreen, size: 13),
  ...
  Widget _verticalDivider() {
    return Container(width: 1, height: 31, color: borderColor);
  }
  ...
  String _getFarmLocation(BuildContext context, Map<String, dynamic> farm) {
    ...
    final farmLocation = farm['farmLocation']?.toString().trim();
