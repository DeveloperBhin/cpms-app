...
class LocalDataService {
  ...
  Future<List<Map<String, dynamic>>> _read(
    ...
    final rows = await database.query(table, where: where, whereArgs: args);
  ...
  Future<List<Map<String, dynamic>>> getFarms() async {
    ...
    try {
      ...
      for (final farm in farms) {
        ...
        if (id != null) {
          await _save('farms', farm, id: id, pending: false);
        }
      }
    } catch (_) {
      // If server is unavailable, return locally cached farms.
    }
  ...
  geometry: values['geometry'],
      ...
      if (id != null) {
        await _save('farms', result, id: id, pending: false);
      }
  ...
  Future<List<Map<String, dynamic>>> getBlocks(String farmId) async {
    ...
    try {
      ...
      // If server is unavailable, return cached blocks.
    }
  ...
  Future<List<Map<String, dynamic>>> getTrees(
    ...
    try {
      ...
      // If server is unavailable, return cached trees.
    }
  ...
  Future<void> syncPending() async {
    ...
    await _syncPendingFarms(database);
    await _syncPendingBlocks(database);
    await _syncPendingTrees(database);
  }
  ...
  Future<void> _syncPendingFarms(Database database) async {
  ...
  geometry: payload['geometry'],
        ...
        // Stop here and retry on the next sync.
        break;
      }
    }
  }
  ...
  Future<void> _syncPendingBlocks(Database database) async {
  ...
  Future<void> _syncPendingTrees(Database database) async {
